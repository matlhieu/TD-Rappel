...
/* --- 1. LES FONCTIONS DE COMPARAISON --- */
...
// Trie et retourne un tableau de chaînes de caractères (Top 10)
char** API_sort_students(Prom* pClass) {
    if (!pClass || !pClass->students || !pClass->current_compare_func) {
        return NULL;
    }

    // 1. On trie le tableau principal en utilisant le pointeur de fonction configuré
    qsort(pClass->students, pClass->numStudent, sizeof(Student), pClass->current_compare_func);

    // 2. Allocation du tableau de chaînes (SIZE_TOP1 = 10)
    ...
    for (int i = 0; i < SIZE_TOP1; i++) {
        if (i < count) {
            ...
            // Formatage selon le PDF (libre choix)
