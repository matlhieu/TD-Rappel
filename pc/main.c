...
/*void test_endianness(Prom *p) { 
    ...
    const char *filename = "student_endian.bin";
    ...
    printf("\n=== Test Endianness (Save %s) ===\n", s->lname);
    printf("Files : %s\n\n", filename);

    fwrite(s->lname, sizeof(char), strlen(s->lname) + 1, f);
    printf("Write Lname : %s (Natural)\n", s->lname);

    fwrite(s->fname, sizeof(char), strlen(s->fname) + 1, f);
    printf("Write Fname : %s (Natural)\n", s->fname);
    }  
*/ 
    ...
    do {
        ...
        switch(choice) {
            ...
            case 9:
                printf("************* Test Ex 1 *************\n"); /*CC 2025*/
                if (promo) {
                    if (miseAJourNotes(promo, normalisation)) {
                        printf("\n Success\n");
                        for(int i=0; i< promo->numStudent; i++) {
                            printf(" - %s %s : %.2f / 20\n", 
                                promo->students[i].lname, 
                                promo->students[i].fname, 
                                promo->students[i].average);
                        }
                        printf("****************************************\n");
                    } else {
                        printf("Failure\n");
            ...
            case 10:                                                        /*CC 2025*/
                if (promo) {
                    int taille = 0;
                    char **noms = listePrenoms(promo, &taille);
                    if (noms) {
                        printf("\n************** List of names *************\n");
